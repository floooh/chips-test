...
//  z80m-zex.c
//
//  Runs Frank Cringle's zexdoc and zexall test through the Z80mini emu. Provide
...
static bool run_test(z80m_t* cpu, const char* name) {
    bool running = true;
    uint64_t ticks = 0;
    uint64_t start_time = stm_now();
    while (running) {
        /* run for a lot of ticks or until HALT is encountered */
        ticks += z80m_exec(cpu, (1<<30));
        /* check for BDOS call */
        const uint16_t pc = z80m_pc(cpu);
        if (5 == pc) {
            if (!cpm_bdos(cpu)) {
                running = false;
            }
...
static bool zexdoc() {
    ...
    z80m_set_sp(&cpu, 0xF000);
    z80m_set_pc(&cpu, 0x0100);
    /* trap when reaching address 0x0000 or 0x0005 */
    z80m_set_trap(&cpu, 0, 0x0000);
    z80m_set_trap(&cpu, 1, 0x0005);
