...
async function deployWebpage(p: Project, srcDir: string, dstDir: string): Promise<void> {
    ...
    for (const item of items) {
        ...
        if (util.fileExists(`${srcDir}/${system}-ui.js`)) {
    ...
    for (const file of ['style.css', 'favicon.png', 'help.html']) {
...
// FIXME TAP: { type:'game', title:"Alien (CPC)",           system:'cpc',   url:'cpc.html?type=cpc464&file=cpc/alien.tap&joystick=true&input=run%22%0A%0A', img:'cpc/alien.jpg', note:''},
    ...
    { type:'hidden', title:"Pacman (Arcade)", system:'pacman', url:'pacman.html', img:'', note:'' },
]
